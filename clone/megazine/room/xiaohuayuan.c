...
void create()
{
	...
	set("long", @LONG
这里〖地狱〗大厦四楼的一个小花园，这里的肥沃土壤十分适合各种
花的生长，特别地一些十分名贵的花种，这里也可生长。你可以在这里
种花(zhonghua)。你可以在西边的花房买到花的种子。
LONG
	);
        ...
        if (me->query_temp("peiyu") == 2)
        {
                if (random(14) == 1)
                {
                        ...
                        if (random(15) == 1)
                        {
                                ...
                                CHANNEL_D->do_channel(this_object(), "rumor",
                                                      me->query("name") + "从小花园种出一朵无名花。");
                                return 1;
                        }

                        message_vision(HIC "$N" HIC "十分小心的给小坑里浇了一些水，突然" +
                                       ob->query("long") + "从坑里冒了出来。\n" NOR,me);   
