...
mixed ask_shennong()
{
        ...
        if (objectp(owner) && owner != this_object())
        {
                ...
                if (owner->query("family/master_id") == query("id"))
                        return "嗯，神农锏现在暂时是你同门师兄弟" + owner->query("name") +
                               "在用，你要用就去找他吧。";
                else
                        return "本门的神农锏现在落入了" + owner->query("name") +
                               "之手，你去把它取回来吧！";
        }
...
mixed ask_shenpian()
{
        ...
        if (objectp(owner) && owner != this_object())
        {
                ...
                if (owner->query("family/master_id") == query("id"))
                        return "嗯，《药王神篇》现在在你同门师兄弟" + owner->query("name") +
                               "手里，你去找他吧。";
                else
                        return "本门的《药王神篇》现在落入了" + owner->query("name") +
                               "之手，你去把它取回来吧！";
        }
